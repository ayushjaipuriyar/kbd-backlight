...
async fn handle_status() -> Result<()> {
    ...
    match response {
        IpcResponse::Status(info) => {
            ...
            println!("Idle:              {}", if info.is_idle { "Yes" } else { "No" });
            println!("Fullscreen:        {}", if info.is_fullscreen { "Yes" } else { "No" });
...
async fn handle_profile(name: String) -> Result<()> {
    let client = IpcClient::new(DEFAULT_SOCKET_PATH);
    let response = client.send_message(&IpcMessage::SetProfile(name.clone())).await?;
...
async fn handle_set(brightness: u32) -> Result<()> {
    let client = IpcClient::new(DEFAULT_SOCKET_PATH);
    let response = client.send_message(&IpcMessage::SetManualBrightness(brightness)).await?;
...
async fn handle_auto() -> Result<()> {
    let client = IpcClient::new(DEFAULT_SOCKET_PATH);
    let response = client.send_message(&IpcMessage::ClearManualOverride).await?;
...
async fn handle_schedule(action: ScheduleAction) -> Result<()> {
    match action {
        ScheduleAction::Add { profile, time, brightness } => {
            ...
            let client = IpcClient::new(DEFAULT_SOCKET_PATH);
            let response = client.send_message(&IpcMessage::AddTimeSchedule {
            ...
            match response {
                IpcResponse::ScheduleAdded => {
                    println!("Added time schedule to profile '{}': {}:{:02} -> brightness {}", 
    ...
    match action {
        DaemonAction::Start => {
            ...
            let output = Command::new("systemctl")
                ...
                .output()
                .map_err(|e| Error::ipc_connection(format!("Failed to execute systemctl: {}", e)))?;
            ...
            if output.status.success() {
                ...
                Err(Error::ipc_connection(format!("Failed to start daemon: {}", stderr)))
            }
        }
        DaemonAction::Stop => {
            ...
            if output.status.success() {
                ...
                let stderr = String::from_utf8_lossy(&output.stderr);
                Err(Error::ipc_connection(format!("Failed to stop daemon: {}", stderr)))
            }
        }
        DaemonAction::Restart => {
            ...
            if output.status.success() {
                ...
                let stderr = String::from_utf8_lossy(&output.stderr);
                Err(Error::ipc_connection(format!("Failed to restart daemon: {}", stderr)))
            }
        }
    }
}
